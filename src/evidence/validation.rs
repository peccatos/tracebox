...
use crate::evidence::store::TracePaths;

/// Machine-readable validation report.
///
/// This type intentionally lives in the evidence layer, not in the CLI command
/// layer. The same validation engine can later be used by:
///
/// - `tracebox validate`;
/// - CI integrations;
/// - embedded runtime integrations;
/// - future remote trace ingestion;
/// - test fixtures that need to assert trace contract correctness.
...
impl ValidationReport {
    pub fn invalid(trace_id: &str, trace_path: &Path, reason: &str) -> Self {
        Self {
...
/// Single semantic validation check.
///
/// `expected`, `actual`, and `detail` are optional so successful checks stay
/// compact in JSON output while failed checks can still explain exactly why the
/// manifest is invalid.
#[derive(Debug, Clone, Serialize)]
...
///
/// This function does not verify artifact bytes. That is the job of
/// `tracebox verify`.
///
/// This function validates the manifest as an evidence contract:
///
/// - schema version;
...
pub fn validate_manifest(
    ...
    let checks = validate_manifest_checks(requested_trace_id, paths, manifest);
    let all_ok = checks.iter().all(ValidationCheck::is_ok);

    ValidationReport {
        trace_id: requested_trace_id.to_string(),
...
mod tests {
    ...
    #[test]
    fn invalid_report_uses_exit_code_2() {
        let report = ValidationReport::invalid(
            "trc_missing",
            std::path::Path::new(".traces/trc_missing"),
            "missing",
        );
